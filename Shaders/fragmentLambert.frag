...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, Material material)
{
...
vec3 CalcDirLight(DirLight light, vec3 normal, Material material)
{
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, Material material)
{
	float attenuation = getAttenuation(light.constant, light.linear, light.quadratic, light.position, fragPos);
...
void main(void){
    ...
    if(dirLight.isDefined == 1){
		result = CalcDirLight(dirLight, norm ,Material(objectColor, shininess));
	}
    for(int i = 0; i < MAX_POINT_LIGHTS; i++)
    {
		if(pointLight[i].isDefined == 1)
            result += CalcPointLight(pointLight[i], norm, ex_worldPosition, Material(objectColor, shininess));
    }
    if(spotLight.isDefined == 1){
        result+= CalcSpotLight(spotLight, norm,  ex_worldPosition,Material(objectColor, shininess));
    }
